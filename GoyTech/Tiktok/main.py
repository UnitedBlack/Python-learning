import json, sys, os, calendar, time, tt_uploader
from video.editor import tt_video_maker
...
from tt_config import tiktok_accounts


def zalu(data):
    account, video_url = data
    download_path = yt_downloader.download_video(url=video_url)
    edited_path = tt_video_maker.edit_video(input_video=download_path)
    tt_uploader.upload_tt(account_name=account, input_file=edited_path)
...
def start():
    count = 0
    schedulers = {name: BackgroundScheduler() for name in tiktok_accounts}
